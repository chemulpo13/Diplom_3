...
class FeedPage(BasePage):
    quantity_of_orders_locator = (By.XPATH, '//p[contains(@class,"OrderFeed_total__")]')
    daily_quantity_of_orders_locator = (By.XPATH, '//p[contains(@class,"OrderFeed_totalToday__")]')
    ...
    def get_quantity_of_orders(self):
        WebDriverWait(self.driver, 10).until(
            EC.invisibility_of_element_located(
                (By.XPATH, "//div[contains(@class,'Preloader') or contains(@class,'Loader')]")
            )
        )
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(FeedPageLocators.quantity_of_orders)
        )
