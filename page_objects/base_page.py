...
class BasePage:
    ...
    @allure.step('Ждать увеличения счетчика по локатору {locator} с {old_value} в течение {timeout} секунд')
    def wait_for_counter_to_increase(self, locator, old_value, timeout=10):
        WebDriverWait(self.driver, timeout).until(
            lambda d: int(d.find_element(*locator).text) > old_value
        )

    @allure.step('Ждать изменения счетчика по локатору {locator} с {old_value} в течение {timeout} секунд')
    def wait_for_counter_to_change(self, locator, old_value, timeout=15):
        WebDriverWait(self.driver, timeout).until(
            lambda d: d.find_element(*locator).text != str(old_value)
        )