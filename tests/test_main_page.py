...
class TestMainPage:
    ...
    def test_navigate_to_order_history_success(self, driver):
        ...
        main_page.click_header_feed_button()
        title = feed_page.get_text_on_title_of_orders_list()
        print(f"DEBUG: Title on feed page: {title!r}")
        assert 'лента заказов' in title.lower()
    ...
    def test_displaying_modal_window_details_of_ingredient_success(self, driver):
        ...
        result = main_page.check_displaying_of_modal_details()
        print("DEBUG: Отображается окно деталей?", result)
        assert result
    ...
    def test_close_modal_window_details_of_ingredient_success(self, driver):
        ...
        closed = main_page.check_not_displaying_of_modal_details()
        print("DEBUG: Окно точно закрылось?", closed)
        assert closed
